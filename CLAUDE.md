...
22. **NEVER investigate code inline** -- The main orchestrator must NEVER search for, read, or trace VB.NET code directly. This applies to ALL scenarios: developer questions, reviewer follow-up (CONCERNS/REVISE verification), ad-hoc investigation, or any other code analysis. ALWAYS launch a sub-agent (Agent tool, subagent_type: "general-purpose") to do the work. The sub-agent gets a fresh context with: the question or concern, file paths, the VB parser path (`vb_parser` from paths.md), and the carrier root. It does all parser calls + file reads, then reports back a concise summary. The main orchestrator presents the findings. Investigating inline fills the context window and triggers compaction, losing critical pipeline state.
23. **Parser mandatory, trace unbounded** -- The VB parser (`vb_parser` from paths.md) is **mandatory** for ALL VB.NET code reading. Never use regex to parse function signatures or boundaries. Never rely on grep alone for structural questions. Always: parser for structure → Read for semantics. `{vb_parser} parse {file}` for file structure, `{vb_parser} function {file} {name}` for function detail. Grep is for file discovery only (finding which files contain a name). When tracing a call chain, follow it wherever it leads — even OUTSIDE the carrier directory into TBW shared framework components (e.g., `Cssi.Net/Components/`, shared engine classes). The carrier repo boundary is NOT a stopping point for investigation. The parser works on any .vb file, not just carrier files.
24. **Dual-impact: calculation + display** -- Every code change has TWO impacts: (a) calculation — what changes in the premium math, and (b) display — what changes on the TBW front-end visible to brokers/underwriters. `AddToArray()` calls build display lines (order = screen order), `AddToDiscountArray()` populates discount breakdowns, running totals appear in the right-hand column. Code block moves change BOTH. Always analyze and verify both impacts, even if the ticket only mentions one.
25. **Investigate thoroughly, no shortcuts** -- Agents must trace as deep as needed to truly understand the code and its implications. There are no artificial depth limits. If an investigation requires reading 20 files across 5 directories, do it. If a call chain goes 8 levels deep into shared framework code, follow every level. Token cost is not a concern — correctness is. Sub-agents have full file system access and should use it exhaustively. A shallow investigation that misses a hidden dependency is far more expensive than a thorough one.
