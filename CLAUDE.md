...
22. **Delegate code questions to a sub-agent** -- When the developer asks about a function, variable, call chain, or code structure (even outside a `/iq-*` command), ALWAYS launch a sub-agent (Agent tool, subagent_type: "general-purpose") to investigate. Never do the investigation inline — it fills the main context window. The sub-agent gets a fresh context with the question, the VB parser path (`vb_parser` from paths.md), and the carrier root. It does all parser calls + file reads, then reports back a concise summary. The main orchestrator presents the findings to the developer.
