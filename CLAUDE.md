...
23. **Parser-first, trace unbounded** -- The VB parser (`vb_parser` from paths.md) is the primary structural tool: `{vb_parser} parse {file}` for file structure, `{vb_parser} function {file} {name}` for function detail. When tracing a call chain, follow it wherever it leads — even OUTSIDE the carrier directory into TBW shared framework components (e.g., `Cssi.Net/Components/`, shared engine classes). The carrier repo boundary is NOT a stopping point for investigation. Use .vbproj `<Compile Include>` and `<Reference>` paths to discover where shared code lives. The parser works on any .vb file, not just carrier files.
